...
function initBurgerMenu() {
  ...
  if (!menuToggle || !nav || !menuContainer) {
    console.warn('Не найдены необходимые элементы для бургер-меню');
    return;
  }

  // 8. СОЗДАЕМ ОВЕРЛЕЙ ДИНАМИЧЕСКИ
  ...
  function toggleMenu() {
    ...
    overlay.classList.toggle('page-header__overlay--active');
  ...
  function closeMenu() {
    ...
    overlay.classList.remove('page-header__overlay--active');
  ...
  // ЗАКРЫТИЕ МЕНЮ ПРИ КЛИКЕ НА ФОН МЕНЮ (если клик не по контенту)
  ...
  // ОБРАБОТКА ИЗМЕНЕНИЯ РАЗМЕРА ОКНА (автоматически закрываем меню при увеличении ширины)
